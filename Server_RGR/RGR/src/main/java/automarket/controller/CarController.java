...
public class CarController extends AbstractController<Car> {
    ...
    public ResponseEntity<?> updateStatus(
        ...
        if (!status.equals("ON_SALE") && !status.equals("BOOKED") && !status.equals("SOLD")) {
            return ResponseEntity
                    .badRequest()
                    .body("{\"error\":\"Недопустимый статус. Допустимые значения: ON_SALE, BOOKED, SOLD\"}");
        }

        service.updateStatus(id, status);
        return ResponseEntity.ok("{\"message\":\"Статус успешно обновлен\"}");
    }
}